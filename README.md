...
| SYS-11 | [`sicsaft-core/`](sicsaft-core) | SICSAFT CORE — app de escritorio nativa | 🟢 Nivel 1 completo (Electron) arrancando de verdad (2026-08-27) — Postgres/Keycloak/CIS/CORE/CIP embebidos con binarios reales vendorizados (sin Redis desde ADR-005), camino prioritario de instalación por cliente junto a `devops/onprem/` (coexisten). Wizard de primer arranque con alta real del Director (`crearUsuarioDirector`, port de `KeycloakAdminService`); falta cablear el paso del Profesional de AFT y el empaquetado final (`electron-builder`) — ver `aidlc-docs/sicsaft-core/` |
