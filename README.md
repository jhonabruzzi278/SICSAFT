...
| SYS-03 | [`core/`](core) | SICSAFT CORE | 🟡 Esqueleto NestJS (mismo patrón que CIS) — ningún motor implementado todavía |
...
/`, `GET /health`, mismos checks) corriendo como servicio interno en el compose local, sin ruta
de Traefik (solo lo consume CIS). **Base Patrimonial** (`base-patrimonial/`) todavía no tiene
código, pero ya tiene el modelo de `Contrato` documentado
([DOC-004](base-patrimonial/DOC-004-modelo-contrato.md)). Lo que sigue faltando: que CORE sirva
`GET /entitlements` sobre ese modelo (reemplazaría el seed fijo que usa CIS hoy), y un cliente
OIDC real (WEB/APP QR) que reemplace los tokens firmados a mano de los tests de CIS.
...
3. `cis/` con el conector QR mockeado — Trello `CIS-ADR-001` — **hecho**, incluye auth real
   contra Zitadel (ADR-002). `core/` tiene su esqueleto básico también (mismo patrón). Sigue
   pendiente que CORE sirva `GET /entitlements` (DOC-004 §6) para que CIS deje de usar el seed
   fijo de `organizaciones`.
