...
El Conector QR es la **única** vía por la que APP QR SICSAFT toca datos que no son locales al dispositivo. La app nunca escribe directo a la BPI — Base Patrimonial Inteligente (ver [ADR-003](./ADR/ADR-003-rename-app-qr-sicsaft.md) y TASK-006). Responsabilidades del conector:
...
Fuera de alcance del conector: aplicar Reglas patrimoniales (eso vive en CORE) y escribir directo en la BPI — Base Patrimonial Inteligente (eso vive en CORE + Reglas).
...
✅ **Resuelto (TASK-007)**: mecanismo real es OIDC contra **Keycloak 26** — authorization code + PKCE, app tipo User Agent/SPA sin secreto de cliente ([ADR-004](../../../adr/ADR-004-identidad-keycloak-reemplaza-zitadel.md), reemplaza al Zitadel de ADR-002). Implementado en `app-qr-sicsaft/src/lib/oidc/`.
