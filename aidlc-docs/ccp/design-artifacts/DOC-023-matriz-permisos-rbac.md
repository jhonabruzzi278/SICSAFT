...
| | Alta / Baja / Reincorporación | ✅ | ➖ | `verificarRolAdministradorPatrimonial` en `OrquestadorService.ejecutarOperacionOficial`; el CIP del Directivo es solo lectura |
| | Modificar responsable / descripción | ✅ | ➖ | ídem |
| **Catálogo Tipo Activo** | Consultar | ⚠️ | ⚠️ | Sin chequeo de rol |
| | Crear | ✅ | ➖ | `verificarRolAdministradorPatrimonial`; el CIP no crea tipos |
| **Documentos de Activo** | Consultar | ⚠️ | ⚠️ | Sin chequeo de rol |
| | Adjuntar / Eliminar | ✅ | ➖ | `verificarRolAdministradorPatrimonial`; el CIP solo consulta documentos |
...
## 4b. Fase 3/6 (reestructuración CCP/CIP, 2026-09-13) — Activos en CIP solo lectura

El catálogo de Activos (consulta, ficha, QR y documentos existentes) se movió del CCP a una
pestaña del CIP (`core/frontend/src/pages/cip/ActivosTab.tsx`). El Directivo puede consultar la
información de su organización, pero el CIP no ofrece alta, baja, reincorporación, edición,
creación de tipos ni adjuntos. Las escrituras oficiales siguen siendo exclusivas de
`administrador-patrimonial` y se verifican contra la organización de la operación.
...
mudaron al CIP, ver 4b) — el CIP es solo lectura y la escritura oficial de Activos sigue siendo
exclusiva de `administrador-patrimonial`. **Nunca** Organizaciones/Contratos/Usuarios de plataforma/Indicadores (esos
