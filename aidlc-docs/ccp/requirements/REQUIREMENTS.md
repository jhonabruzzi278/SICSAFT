...
> **Nota (2026-09-17)**: esta tabla documenta requisitos de cuando "WEB" era un portal único
> (antes de DOC-022). El **Módulo Activos** (RF-03, RF-08, RF-11, RF-12, RF-13) se implementó acá,
> pero **el 2026-09-13 se mudó por completo de `ccp/` a `core/frontend/src/pages/cip/ActivosTab.tsx`**
> (parte del CIP, ver DOC-035) — hoy no vive en el portal del Profesional de AFT. El estado
> "✅ Implementado" de esas filas sigue siendo cierto, pero el **dónde** cambió; no lo busques en
> `ccp/`. Mismo movimiento para el módulo QR/Etiquetas (extraído a `herramientas/generador-qr/`,
> ver DOC-029 "Tabla vigente").

## Funcionales

| ID | Requisito | Estado | Fuente |
|---|---|---|---|
| RF-01 | Login vía Keycloak 26 (authorization code + PKCE), migrado de Zitadel según ADR-004 — sin credenciales propias del portal. | ✅ Implementado, verificado con login real de navegador | ADR-004, `devops/README.md` |
| RF-02 | Hub post-login muestra solo los módulos habilitados por el contrato vigente del operador — nunca todos los módulos a todos los usuarios. | ✅ Implementado | ADR-004 flujo de login, DOC-004 5 (`modulosContratados`) |
...
| RF-10 | Segmentación por rol: el Directivo (rol Keycloak `directivo`, nuevo) aterriza directo en el Dashboard de su organización sin pasar por el hub operativo — no ve Activos/Contratos/Inventarios/Estructura. El profesional de AFT (sin rol especial) sigue viendo el hub operativo actual, sin cambios. | ✅ Implementado y verificado real de punta a punta, los 3 casos — `esDirectivo()` en `oidc-client.ts` + bifurcación en `HubPage.tsx`; logins reales contra Keycloak confirman: Directivo puro → redirect a `/dashboard`, Directivo+administrador-patrimonial → hub completo, solo administrador-patrimonial → hub completo sin cambios | DOC-019 2/7 (decisión diferida que DOC-020 resuelve), DOC-013 4, DOC-020 |
| RF-11 | Módulo Activos gana ciclo de vida completo desde WEB: baja, reincorporación, cambio de responsable y edición de descripción — cierra el gap "estados"/"descripciones" de DOC-012. | ✅ Implementado — verificado real de punta a punta contra Keycloak y Postgres | DOC-012 "Cobertura real desde el CCP hoy", DOC-021 |
| RF-12 | Catálogo de tipos de activo (familias/categorías) gestionable desde WEB — reemplaza el campo de texto libre `catalogoId` por un selector real alimentado por `GET /catalogo-tipos`, con alta de tipos nuevos. | ✅ Implementado — verificado real de punta a punta contra Keycloak y Postgres | DOC-012 "Cobertura real desde el CCP hoy", DOC-021 |
| RF-13 | Documentación y fotografías por Activo (versión mínima: enlace externo + metadata, sin bucket/OCR propio todavía) — alta, listado y baja. | ✅ Implementado — verificado real de punta a punta contra Keycloak y Postgres | DOC-012 "Cobertura real desde el CCP hoy", DOC-021 |
| RF-14 | Importaciones controladas desde archivo (CSV) — módulo nuevo `ImportacionesPage`, parseo cliente-side, preview y resultado por fila contra `POST /importaciones/contable` (ya existía en CORE, sin puente ni UI). | ✅ Implementado — verificado real de punta a punta contra Keycloak y Postgres | DOC-012 "Cobertura real desde el CCP hoy", DOC-021 |
