...
raíz (`cis/`, `core/`, `web/`, `web_admin/`, ...) es su propio desplegable con su propio
...
**Tres portales, tres roles, tres logins — nunca uno compartido** (DOC-022): `web/` (pendiente de
renombrar a `ccp/`) es exclusivo del Profesional de AFT, `web_admin/` del Administrador del
Sistema, y `core/frontend/` del Directivo. `core/` es el único sistema con dos deployables
(`core/` = backend NestJS, `core/frontend/` = SPA) — el frontend le habla a CIS, nunca a CORE
directo (ver [ADR-003](adr/ADR-003-frontend-de-core-para-directivo.md)), así que la regla de abajo
