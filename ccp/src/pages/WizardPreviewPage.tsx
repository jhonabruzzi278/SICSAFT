...
export function WizardPreviewPage() {
  ...
  useEffect(() => {
    void Promise.all([
      QRCode.toDataURL(`https://${lanIp}:8765`, { width: 175, margin: 1, color: { dark: '#0F172A', light: '#FFFFFF' } }),
      QRCode.toDataURL(`https://${lanIp}:8765/sicsaft-aft.apk`, { width: 175, margin: 1, color: { dark: '#0F172A', light: '#FFFFFF' } }),
    ]).then(([pwa, apk]) => {
      setQrPwa(pwa);
      setQrApk(apk);
    });
  ...
  function completarInstalacion(rol: 'administrador-patrimonial' | 'directivo') {
    const email = rol === 'administrador-patrimonial' ? aftEmail : directorEmail;
    const nombre = rol === 'administrador-patrimonial' ? 'Profesional AFT' : 'Director General';

    const header = btoa(JSON.stringify({ alg: 'none', typ: 'JWT' })).replace(/=/g, '');
    const payload = btoa(
      JSON.stringify({
        name: `${nombre} (${clienteNombre})`,
        sub: `user-${email}`,
    ...
    navigate(`/activos?organizacionId=${encodeURIComponent(organizacionId || 'duoc-uc')}`);
  }
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between p-4 sm:p-6 bg-[radial-gradient(60rem_40rem_at_50%_-10%,oklch(30%_0.09_252/0.5),transparent_70%)]">
            ...
            <div className="space-y-5">
              <div>
                ...
                <p className="mt-1 text-xs text-text-dim">
                  Configuración inicial del cliente, sede principal y nivel contratado en la Base Patrimonial Inteligente (BPI).
                </p>
              </div>

              <div className="space-y-4">
                <div>
                  <Label htmlFor="clienteNombre">Nombre de la Organización / Cliente</Label>
                  <Input
                    id="clienteNombre"
                    value={clienteNombre}
                ...
                <div>
                  <Label htmlFor="organizacionId">Identificador único (Slug DNS-Safe)</Label>
                  <Input
                    id="organizacionId"
                  ...
                  <span className="text-[11px] text-text-dim mt-0.5 block">
                    Se autocompleta automáticamente. Solo minúsculas, números y guiones.
                  </span>
                </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1.5">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-xs text-text">Nivel 1 — Modo Básico</span>
                        {nivel === 1 && <Badge variant="success">SELECCIONADO</Badge>}
                      </div>
                      <p className="text-[11px] text-text-dim mt-1">
                        APP QR + CCP completo (gestión de activos, fichas técnicas, etiquetas, auditoría e importaciones).
                      </p>
                    </div>
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-xs text-accent-strong">Nivel 2 — Modo Profesional</span>
                        {nivel === 2 && <Badge variant="success">SELECCIONADO</Badge>}
                      </div>
                      <p className="text-[11px] text-text-dim mt-1">
                        Todo el Nivel 1 + <strong>CIP (Centro de Inteligencia Patrimonial)</strong> con analítica y KPIs en tiempo real.
                      </p>
                    </div>
                  </div>
                </div>
          ...
          {pasoActual === 2 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="mt-1 text-xs text-text-dim">
                  Credencial de acceso generada con rol <strong>Directivo</strong> para supervisión ejecutiva y designación.
                </p>
              </div>

              <div className="space-y-4">
                <div>
                  <Label htmlFor="directorEmail">Correo Electrónico del Director</Label>
                  <Input
                    id="directorEmail"
                    value={directorEmail}
                ...
                <p className="text-xs text-text-dim">
                  El Director utilizará esta credencial inicial para supervisar contratos, autorizar bajas críticas y acceder al portal ejecutivo.
                </p>
          ...
          {pasoActual === 3 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="mt-1 text-xs text-text-dim">
                  Credencial generada con rol <strong>Administrador Patrimonial</strong> para gestionar el catálogo y relevamientos.
                </p>
              </div>

              <div className="space-y-4">
                <div>
                  <Label htmlFor="aftEmail">Correo Electrónico del Profesional AFT</Label>
                  <Input
                    id="aftEmail"
                    value={aftEmail}
                ...
                <p className="text-xs text-text-dim">
                  Con este rol, el operador administrará el Centro de Control Patrimonial (CCP), cargará planillas por Python ETL y sincronizará la APP QR en terreno.
                </p>
          ...
          {pasoActual === 4 && (
            <div className="space-y-5">
              <div className="text-center">
                ...
                <p className="mt-1 text-xs text-text-dim">
                  La organización <strong className="text-text">{clienteNombre}</strong> ha sido provisionada exitosamente en modo {nivel === 2 ? 'Nivel 2 (Profesional con CIP)' : 'Nivel 1 (Básico)'}.
                </p>
              </div>
              ...
              <div className="rounded-xl border border-border bg-bg-raised p-4 space-y-3">
                  ...
                  <div className="inline-flex rounded-lg border border-border bg-bg-card p-0.5 text-xs">
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-md font-semibold transition-all ${
                        tabQr === 'pwa' ? 'bg-accent text-bg shadow-sm' : 'text-text-dim hover:text-text'
                      }`}
                    >
                      PWA Web (Cámara)
                    </button>
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-md font-semibold transition-all ${
                        tabQr === 'apk' ? 'bg-accent text-bg shadow-sm' : 'text-text-dim hover:text-text'
                      }`}
                    >
                      APK Android (.apk)
                    </button>
                ...
                <div className="flex flex-col items-center justify-center p-3 bg-white rounded-xl shadow-inner">
                  {tabQr === 'pwa' && qrPwa && (
                    <img src={qrPwa} alt="QR PWA" className="h-36 w-36 object-contain" />
                  )}
                  {tabQr === 'apk' && qrApk && (
                    <img src={qrApk} alt="QR APK" className="h-36 w-36 object-contain" />
                  )}
                </div>

                <div className="text-center">
                  <span className="text-xs font-mono font-bold text-accent-strong">
                    https://{lanIp}:8765{tabQr === 'apk' ? '/sicsaft-aft.apk' : ''}
                  </span>
              ...
              <div className="space-y-2 pt-2">
                <Button
                  ...
                  onClick={() => completarInstalacion('administrador-patrimonial')}
                >
                  🚀 Ingresar al Portal como Profesional AFT{aftEmail ? ` (${aftEmail})` : ''}
                </Button>
                <Button
                  type="button"
                  variant="secondary"
                  className="w-full !py-2 text-xs font-semibold"
                  onClick={() => completarInstalacion('directivo')}
                >
                  🏛️ Ingresar como Director{directorEmail ? ` (${directorEmail})` : ''}
                </Button>
              </div>
      ...
      <footer className="text-center text-xs text-text-dim max-w-4xl mx-auto w-full border-t border-border/40 pt-4">
        SICSAFT Enterprise Asset Intelligence • Protocolo OIDC / PKCE • Base Patrimonial Inmutable (BPI)
      </footer>
    </div>
  );
}
