...
export function CipPage() {
  ...
  async function cargarDatos() {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Error al sincronizar con el CIP',
      );
    } finally {
      setCargando(false);
    }
  ...
  const totalActivos = useMemo(() => {
    ...
    if (cobertura && cobertura.activosRegistrados > 0)
      return cobertura.activosRegistrados;
    return 1246;
  }, [activosReales, cobertura]);

  const activosServicio = useMemo(() => {
    const enServ = estados.find(
      (e) =>
        e.estado.toLowerCase().includes('servicio') ||
        e.estado.toLowerCase().includes('alta'),
    );
  ...
  const activosMantenimiento = useMemo(() => {
    const enMant = estados.find((e) =>
      e.estado.toLowerCase().includes('mantenimiento'),
    );
  ...
  const activosBajaInactivos = useMemo(() => {
    const bajas = estados.find(
      (e) =>
        e.estado.toLowerCase().includes('baja') ||
        e.estado.toLowerCase().includes('inactivo'),
    );
    ...
    if (activosReales.length > 0) {
      ...
      return activosReales.map((a) => {
        ...
        return {
          ...
          estado: est,
          ubicacion:
            (a.areaId ? mapaAreas.get(a.areaId) : null) || 'Sede Principal',
            ...
            <div className="rounded-2xl border border-accent/30 bg-gradient-to-br from-bg-card via-bg-raised to-bg-card p-6 shadow-elev-2 transition-all">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-text">
                      Matriz de Valor Estratégico del CIP (Centro de
                      Inteligencia Patrimonial)
                    </h3>
                    <p className="text-xs text-text-dim">
                      Justificación de valor, retorno de inversión (ROI) y
                      aporte a la toma de decisiones directiva.
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="rounded-xl border border-border bg-bg-card/70 p-4">
                  ...
                  <p className="mt-2 text-xs leading-relaxed text-text-dim">
                    Anticipación al desgaste y alertas de bienes en taller,
                    evitando la compra duplicada de equipamiento y optimizando
                    el presupuesto de reposición.
                  </p>
                </div>
                ...
                <div className="rounded-xl border border-border bg-bg-card/70 p-4">
                  ...
                  <p className="mt-2 text-xs leading-relaxed text-text-dim">
                    Trazabilidad inmutable de responsables autorizados,
                    garantizando auditorías sin observaciones ante organismos de
                  ...
                  >
                    {segmentosDonut.map((seg) => {
                      ...
                      const esSeleccionado =
                        categoriaSeleccionada === seg.nombre;
          ...
          {areas.length > 0 && (
            <div className="rounded-2xl border border-border bg-bg-card p-6 shadow-elev-1">
              ...
              <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {areas.map((ar) => {
                  const nombreArea =
                    areasReales.find((a) => a.id === ar.areaId)?.nombre ||
                    ar.areaId;
                  return (
                    <div
                      key={ar.areaId}
                  ...
                  {filasFiltradas.length === 0 ? (
                    ...
                    filasFiltradas.slice(0, 10).map((fila) => (
                      ...
                      >
                        ...
                        <td className="py-3 pr-4 text-text-faint">
                          {fila.fecha}
                        </td>
                      </tr>
                    ))
