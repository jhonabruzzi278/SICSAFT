    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Proporción de bienes escaneados y confirmados en terreno respecto al
            padrón total.
          </p>
        </div>
        ...
        <div className="my-6 flex flex-col items-center justify-center">
          ...
          <div className="mt-2 text-center text-xs text-slate-600">
            <span className="font-semibold text-slate-900">
              {cobertura?.activosEscaneados?.toLocaleString() ?? 0}
            </span>{' '}
            de{' '}
            <span className="font-semibold text-slate-900">
              {cobertura?.activosRegistrados?.toLocaleString() ?? 0}
            </span>{' '}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col justify-between">
        ...
        <div className="my-4 space-y-3">
          {(() => {
            const totalCat =
              categorias.reduce((sum, c) => sum + c.cantidad, 0) || 1;
            return categorias.slice(0, 5).map((cat, idx) => {
              const color = paletaCategorias[idx % paletaCategorias.length];
              const nombre = cat.familia || 'Sin familia';
              const porcentaje = (cat.cantidad / totalCat) * 100;
              return (
                <div key={`${cat.areaId}-${nombre}`} className="space-y-1">
                  <div className="flex justify-between text-xs">
                    ...
                    <span className="font-semibold text-slate-900">
                      {cat.cantidad.toLocaleString()} ({porcentaje.toFixed(1)}%)
                    </span>
                  </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col justify-between">
        ...
        <div className="my-4 space-y-3">
          {(() => {
            const totalEst =
              estados.reduce((sum, e) => sum + e.cantidad, 0) || 1;
            return estados.map((est) => {
              const porcentaje = (est.cantidad / totalEst) * 100;
              return (
                <div
                  key={est.estado}
