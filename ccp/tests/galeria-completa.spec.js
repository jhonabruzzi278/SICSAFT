...
test('07 - CCP Resumen Operativo con Botón Destacado al CIP', async ({
  page,
}) => {
  await seedAuth(page);
...
test('08 - CIP Web Interactiva: 4 Stat Cards, Donut Chart y Bar Chart SVG', async ({
  page,
}) => {
  await seedAuth(page);
...
test('09 - CIP Web Interactiva: Activos Recientes y Matriz de Valor', async ({
  page,
}) => {
  await seedAuth(page);
  await page.goto('/cip?organizacionId=duoc-uc');
