...
del desarrollador) y barre procesos huérfanos de una corrida anterior. `global-teardown` restaura
`%APPDATA%` (salvo `KEEP_APPDATA=1`).

El `.exe` se lanza con **`_electron.launch`** (no `chromium.connectOverCDP`: contra este build de
Electron el handshake CDP se cuelga). En `principal` (specs 01-11) hay **un solo `.exe` vivo** vía
la fixture worker `exe`; el teardown de esa fixture NO espera indefinidamente el `app.close()` (se
colgaba 120s porque el `.exe` deja huérfano el `java` de Keycloak en Windows y su pipe queda
abierto): tope de 15s + `taskkill /T` del árbol. Ver `fixtures/electron.ts` y
`scripts/exe-process.ts`.

Tres frentes (`workers: 1`, en serie):

- **Electron** (la ventana del renderer, `_electron`) — arranque, wizard, single-instance,
  watchdog, relanzamiento, cierre. Assert vía IPC (`page.evaluate(() => window.sicsaftCore.*)`),
  `app.evaluate`/`app.windows()` y el log / estado del proceso.
...
Dos _projects_: `principal` (specs 01-11, contra la instancia de la fixture worker) y `ciclo-vida`
(12-13, `dependencies: ['principal']` → corre después de que esa instancia se cerró; estas specs
lanzan su propia instancia con `_electron.launch`).
