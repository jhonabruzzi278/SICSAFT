...
| RF-05 | El Motor Patrimonial resuelve consulta de activos, cambio de ubicación/estado y traslado — no alta/baja/reincorporación/cambio de responsable (Fase 4). | ⚠️ Parcial — consulta ✅; traslado y cambio de ubicación/estado tienen el repository listo pero **sin controller HTTP** (sin consumidor real todavía, documentado como YAGNI, no como pendiente urgente) | `core/README.md` § Arquitectura interna, ROADMAP.md Fase 2 |
...
| ID | Requisito | Estado | Fuente |
|---|---|---|---|
| RNF-01 | Ningún endpoint de listado devuelve un dataset sin paginar. | ⚠️ Parcial — `GET /catalogo` pagina; los listados agregados en Fase 5 (`GET /contratos`, `GET /auditoria`, `GET /areas`, `GET /ubicaciones`, `GET /responsables`) **no paginan**, aceptado por volumen bajo y documentado explícitamente en cada uno (mismo criterio ya aplicado a `GET /contratos` desde que se creó) | `ARQUITECTURA-WAF.md` §5 |
