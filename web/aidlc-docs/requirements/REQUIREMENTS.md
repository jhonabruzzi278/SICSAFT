...
| RF-06 | Módulo Auditoría: solo lectura, **filtrable por usuario/fecha/operación**. | ⚠️ **Parcial** — lectura de las 200 entradas más recientes implementada; **sin ningún filtro**, ni server-side (CORE) ni client-side (WEB). El requisito pide filtros explícitos y no se construyeron en este incremento. | DOC-011 (Motor de Auditoría) |
...
Distinto de "Fuera de alcance" (abajo, nunca se pidió): estos dos SÍ están en el alcance de RF-05/RF-06
y quedaron sin resolver en el incremento que los construyó — anotado acá para que no se pierdan en
el histórico del ROADMAP.
...
- **RF-06**: sin filtro por usuario/fecha/operación (ni backend ni UI). Requiere agregar parámetros
  de query a `GET /auditoria` en CORE (`AuditoriaRepository.listar`) y los controles
  correspondientes en `AuditoriaPage`.
