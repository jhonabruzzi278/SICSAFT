...
| RF-11 | Módulo Activos gana ciclo de vida completo desde WEB: baja, reincorporación, cambio de responsable y edición de descripción — cierra el gap "estados"/"descripciones" de DOC-012. | 🟡 En construcción (2026-08-18) | DOC-012 § "Cobertura real desde el CCP hoy", DOC-021 |
| RF-12 | Catálogo de tipos de activo (familias/categorías) gestionable desde WEB — reemplaza el campo de texto libre `catalogoId` por un selector real alimentado por `GET /catalogo-tipos`, con alta de tipos nuevos. | 🟡 En construcción (2026-08-18) | DOC-012 § "Cobertura real desde el CCP hoy", DOC-021 |
| RF-13 | Documentación y fotografías por Activo (versión mínima: enlace externo + metadata, sin bucket/OCR propio todavía) — alta, listado y baja. | 🟡 En construcción (2026-08-18) | DOC-012 § "Cobertura real desde el CCP hoy", DOC-021 |
| RF-14 | Importaciones controladas desde archivo (CSV) — módulo nuevo `ImportacionesPage`, parseo cliente-side, preview y resultado por fila contra `POST /importaciones/contable` (ya existía en CORE, sin puente ni UI). | 🟡 En construcción (2026-08-18) | DOC-012 § "Cobertura real desde el CCP hoy", DOC-021 |
| RF-15 | Rol nuevo Administrador del Sistema (`administrador-sistema`): módulo "Administración" en WEB para crear organizaciones, crear/editar contratos (además del Profesional de AFT), asignar usuarios a organizaciones (integración real con la API de administración de Zitadel) y ver indicadores de plataforma — nunca toca información patrimonial (Activos/Catálogo/Documentos siguen exclusivos de `administrador-patrimonial`). | 🟡 En construcción (2026-08-18) | DOC-021 |
